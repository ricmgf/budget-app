...
const BudgetGrid = {
  ...
  _parseIntessa(rows) {
    ...
    // ── Consolidate commissions with their parent operations ──
    // Commissions (Commissioni, Costo Bonifico, Commissione) that appear right after
    // a parent operation get merged into the parent with a breakdown
    const FEE_PATTERNS = ['COMMISSIONI E SPESE', 'COMMISSIONE DISPOSIZIONE', 'COMMISSIONE PAGAMENTO',
      'COSTO BONIFICO'];
    const consolidated = [];
    const used = new Set();
    ...
    for (let i = 0; i < mvs.length; i++) {
      ...
      // Skip if this IS a fee row — it will be absorbed by its parent
      if (FEE_PATTERNS.some(p => cUp.includes(p))) continue;

      // Look for fee rows immediately after this one (i+1, i+2) with same or next date
      const children = [];
      for (let j = i + 1; j < mvs.length && j <= i + 2; j++) {
        if (used.has(j)) continue;
        const candidate = mvs[j];
        const candUp = candidate.concepto.toUpperCase();
        if (FEE_PATTERNS.some(p => candUp.includes(p)) && candidate._rawAmount < 0) {
          children.push(candidate);
          used.add(j);
        } else {
          break; // stop looking once we hit a non-fee row
        }
      }

      if (children.length > 0) {
        // Build breakdown: parent + fees
        const totalAmount = mv.amount + children.reduce((s, c) => s + c.amount, 0);
        const breakdownItems = [
          { label: mv.concepto.substring(0, 40), amount: mv.amount },
          ...children.map(c => ({ label: c.concepto.substring(0, 40), amount: c.amount }))
        ];
        const breakdownStr = breakdownItems.map(b => `${b.label}: ${b.amount.toFixed(2)}`).join('\n');
        consolidated.push({
          concepto: mv.concepto,
          amount: totalAmount,
          originalSign: mv.originalSign,
          date: mv.date,
          notes: mv.notes + (mv.notes ? ' | ' : '') + 'Incl. ' + children.map(c => c.concepto.substring(0, 30) + ': ' + c.amount.toFixed(2)).join(', '),
          _breakdown: breakdownStr
        });
        console.log(`[Intessa] Consolidated: '${mv.concepto}' + ${children.length} fees → ${totalAmount.toFixed(2)}`);
      } else {
        consolidated.push({ concepto: mv.concepto, amount: mv.amount, originalSign: mv.originalSign, date: mv.date, notes: mv.notes });
      }
    }

    console.log(`[Intessa] Final: ${consolidated.length} movements (from ${mvs.length} raw)`);
    return consolidated;
  },
  ...
  _parseAmexPDF(text) {
    ...
    for (let idx = 0; idx < anchors.length; idx++) {
      ...
      cleanBlock = cleanBlock.replace(/Cambio aplicado[\s\S]*/i, '').trim();
      cleanBlock = cleanBlock.replace(/Itinerario[\s\S]*/i, '').trim();
      cleanBlock = cleanBlock.replace(/Llegada[\s\S]*/i, '').trim();
      cleanBlock = cleanBlock.replace(/Nº de billete[\s\S]*/i, '').trim();
