  ...
  async _parseSmartXLSX(file) {
    ...
    for (const sn of wb.SheetNames) {
      ...
      for (let i = 0; i < Math.min(rows.length, 40); i++) {
        ...
        if (cellJoined.includes('FECHA OPERACIÓN') || (cellJoined.includes('COMERCIO') && cellJoined.includes('IMPORTE EUROS'))) { console.log('[Detection] IBERIA at row', i, 'sheet', sn); return this._parseIberia(rows); }
        if (cellJoined.includes('MOVIMIENTO') && (cellJoined.includes('MÁS DATOS') || cellJoined.includes('MAS DATOS') || cellJoined.includes('IMPORTE'))) { console.log('[Detection] CAIXA at row', i, 'sheet', sn); return this._parseCaixa(rows); }
  ...
  _parseIberia(rows) {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (colBUp.includes('IBERIA') && r[4]) {
        ...
        console.log(`[Iberia] Titular found: ${currentTitular} at row ${i}`);
        continue;
      }
      // Skip header rows
      ...
      if (!concepto || Math.abs(amount) < 0.01) {
        console.log(`[Iberia] Skip no-amt: row ${i} concepto='${concepto}' amt=${amount}`);
        continue;
      }
      if (!currentTitular) { console.log(`[Iberia] Skip no-titular: row ${i}`); continue; }
      // Skip concepto-level summary words
      const cUp = concepto.toUpperCase();
      if (SKIP_CONCEPTO.some(w => cUp.includes(w))) {
        console.log(`[Iberia] Skip keyword: row ${i} '${concepto}'`);
        continue;
      }
