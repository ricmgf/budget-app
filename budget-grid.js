...
const BudgetGrid = {
  ...
  render() {
    ...
    console.log(`[render] GASTOS: ${G.length} lines, hasParents=${gHasParents}, parents=${G.filter(l=>!l.parentId).length}, children=${G.filter(l=>l.parentId).length}`);
  ...
  _tarjetaRows(lines, cm) {
    ...
    parents.forEach(line => {
      ...
      if (isParent) {
        console.log(`[_tarjetaRows] Parent "${line.concepto}" (${line.id}): ${kids.length} kids, collapsed=${collapsed}`);
        if (!collapsed && kids.length > 0) {
          console.log(`[_tarjetaRows] First 3 kids:`, kids.slice(0,3).map(k => `${k.id}="${k.concepto}"`));
          console.log(`[_tarjetaRows] Last 3 kids:`, kids.slice(-3).map(k => `${k.id}="${k.concepto}"`));
        }
  ...
  openDrawer(lineIdOrEvent) {
    ...
    if (!lineId) { console.warn('[openDrawer] No lineId'); return; }
    console.log(`[openDrawer] Requested ID: ${lineId}`);
    const line = this.lines.find(l => l.id === lineId);
    if (!line) {
      console.warn(`[openDrawer] Line not found: ${lineId}. Total lines: ${this.lines.length}`);
      this._toast('Línea no encontrada. Recarga la página.');
      return;
    }
    console.log(`[openDrawer] Found: "${line.concepto}" (id=${line.id}, parentId=${line.parentId||'none'})`);
  ...
  async _impConfirm(type) {
    ...
    for (const mv of movements) {
      ...
      if (type === 'tarjeta') {
        ...
        const group = GROUP_RULES.find(g => cUp.includes(g.pattern));
        ...
        if (hasTitular) {
          ...
          const bankGroup = GROUP_RULES.find(g => cUp.includes(g.pattern));
        ...
        if (bankGroup) {
          ...
          const normKey = this._norm(rawC) + '|||' + this._norm(enrichedNotes) + '|||' + section;
