...
const BudgetGrid = {
    ...
    ov.innerHTML = `<div class="budget-drawer">
      ...
      <div class="dw-group-section">
        <div class="dw-group-header" onclick="document.getElementById('dw-group-body').classList.toggle('hidden');BudgetGrid._grpRenderCtx('${line.id}')">
          <span>🔗 Agrupación</span><span class="dw-group-chevron">▸</span>
        </div>
  ...
  _grpRenderCtx(lineId) {
    ...
    // Check if this line's concepto matches any group rule
    const matchingRule = BudgetLogic._groupRules.find(r =>
      (!r.bank || r.bank === bank) && cUp.includes(r.pattern)
    );
    ...
    if (matchingRule) {
      ...
      container.innerHTML = `<div class="dw-grp-empty">Este concepto no pertenece a ningún grupo.</div>
        <div style="margin-top:8px;">
          ...
          <div style="font-size:10px;color:var(--text-tertiary);margin:4px 0;" id="dw-grp-preview"></div>
          <button class="btn-save" style="width:100%;margin-top:8px;" onclick="BudgetGrid._grpCreateNew('${lineId}')">Crear grupo</button>
        </div>`;
      // Live preview of matching lines
      const patInput = document.getElementById('dw-grp-new-pattern');
      if (patInput) {
        const updatePreview = () => {
          const q = patInput.value.toUpperCase().trim();
          if (q.length < 2) { document.getElementById('dw-grp-preview').textContent = ''; return; }
          const count = this.lines.filter(l => l.bank === bank && l.concepto && l.concepto.toUpperCase().includes(q)).length;
          document.getElementById('dw-grp-preview').textContent = count ? `→ ${count} línea${count > 1 ? 's' : ''} matchean este patrón` : 'Sin coincidencias';
        };
