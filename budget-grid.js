...
const BudgetGrid = {
  ...
  // Preview with checkboxes — wire AFTER innerHTML so listeners survive re-renders
  _grpAttachPreviewChecked(searchEl, previewEl, patternEl, bank) {
    const MIN = 2;
    const showHint = () => {
      previewEl.innerHTML = '<p style="font-size:11px;color:var(--text-tertiary);margin:0;padding:4px 2px;">Escribe para buscar líneas del extracto...</p>';
    };
    showHint();

    // Event delegation on the wrap — survives innerHTML replacements
    previewEl.addEventListener('change', () => {
      const selected = [...previewEl.querySelectorAll('input[type=checkbox]:checked')].map(c => c.value);
    ...
    searchEl.oninput = () => {
      ...
      if (q.length < MIN) { showHint(); return; }
      previewEl.innerHTML = '<p style="font-size:11px;color:var(--text-tertiary);margin:0;padding:4px 2px;">Buscando...</p>';
      _timer = setTimeout(async () => {
        const lines = await BudgetLogic.searchGastos(q, bank);
        if (!lines.length) {
          previewEl.innerHTML = '<p style="font-size:11px;color:var(--text-tertiary);margin:0;padding:4px 2px;">Sin coincidencias</p>';
          return;
        }
        // Build result items — simple divs with native checkboxes, NO onclick on label
        let html = '<p style="font-size:11px;color:var(--text-tertiary);margin:0 0 6px;padding:0 2px;">'
          + lines.length + ' línea' + (lines.length !== 1 ? 's' : '')
          + ' · selecciona para añadir como patrón</p>';
        ...
        lines.slice(0, 15).forEach(l => {
          ...
          const label = this._e(l.alias || l.concepto);
          const val   = this._e(l.concepto.toUpperCase());
          const sub   = l.notas ? '<div style="font-size:10px;color:var(--text-tertiary);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;margin-top:2px;">' + this._e(String(l.notas).slice(0, 55)) + '</div>' : '';
          // Use a <label> that wraps the checkbox — native toggle, no onclick needed
          html += '<label style="display:flex;align-items:flex-start;gap:8px;'
            + 'padding:7px 8px;margin-bottom:3px;border-radius:6px;cursor:pointer;'
            + 'background:#f8f9fa;border:1px solid var(--border-light);'
            + 'font-size:12px;line-height:1.3;user-select:none;">'
            + '<input type="checkbox" value="' + val + '" style="margin-top:3px;flex-shrink:0;cursor:pointer;accent-color:var(--accent);">'
            + '<div style="flex:1;min-width:0;">'
            +   '<div style="display:flex;justify-content:space-between;gap:6px;">'
            +     '<span style="font-weight:600;color:var(--text-primary);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">' + label + '</span>'
            +     '<span style="font-weight:600;color:var(--accent);flex-shrink:0;">' + amt + '</span>'
            +   '</div>'
            +   sub
            + '</div>'
            + '</label>';
        ...
        if (lines.length > 15) {
          html += '<p style="font-size:11px;color:var(--text-tertiary);margin:4px 0 0;padding:0 2px;">+' + (lines.length - 15) + ' más...</p>';
        }
        previewEl.innerHTML = html;
        // Re-sync patternEl on next check cycle (delegation on previewEl handles it)
      }, 250);
    };
  },
  ...
  _grpRenderCtx(lineId) {
    ...
    if (matchingRule) {
      ...
      html += `<div class="dw-grp-add-section">
          ...
          <div style="display:flex;gap:6px;margin-top:6px;">
            <input class="dw-grp-pattern-input" placeholder="Patrón a guardar..." data-label="${this._e(label)}"
              style="flex:1;padding:7px 10px;border:1px solid var(--border-light);border-radius:6px;font-size:12px;min-width:0;">
            <button class="dw-grp-add-btn" onclick="BudgetGrid._grpAddPattern(this,'${lineId}')" style="padding:7px 14px;background:var(--accent);color:white;border:none;border-radius:6px;cursor:pointer;font-size:12px;font-weight:600;">+</button>
          </div>
        </div>
      </div>`;
      ...
      if (searchInput && previewWrap) {
        this._grpAttachPreviewChecked(searchInput, previewWrap, patternInput, bank);
      ...
      // Live preview using shared BudgetLogic.searchGastos
      const patInput = document.getElementById('dw-grp-new-pattern');
      const pvEl     = document.getElementById('dw-grp-preview');
      if (patInput && pvEl) this._grpAttachPreview(patInput, pvEl, bank);
    }
  },
