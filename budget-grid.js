...
const BudgetGrid = {
  ...
  _grpAttachPreview(inputEl, previewEl, bank) {
    const MIN = 2;
    const hint = `<span style="font-size:10px;color:var(--text-tertiary);">Escribe ${MIN}+ letras para buscar en transacciones</span>`;
    ...
    inputEl.oninput = () => {
      ...
      _timer = setTimeout(async () => {
        const results = await BudgetLogic.searchGastos(q, bank);
        if (!results.length) {
          previewEl.innerHTML = '<span style="font-size:10px;color:var(--text-tertiary);">Sin coincidencias</span>';
          return;
        }
        const total = results.reduce((s,r) => s + r.count, 0);
        let html = `<div style="font-size:10px;color:var(--text-tertiary);margin-bottom:4px;">${total} transacción${total!==1?'es':''} encontradas</div>`;
        results.slice(0, 15).forEach(r => {
          const amt = r.total.toLocaleString('es-ES', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
          html += `<div style="display:flex;justify-content:space-between;align-items:center;
            padding:4px 8px;background:var(--bg-secondary);border:1px solid var(--border-light);
            border-radius:4px;margin-bottom:2px;font-size:11px;gap:8px;">
            <span style="color:var(--text-primary);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${this._e(r.concepto)}</span>
            <span style="color:var(--text-tertiary);flex-shrink:0;">${r.count>1?r.count+'× ':''}<strong style="color:var(--text-secondary);">${amt}</strong></span>
          </div>`;
        });
        if (results.length > 15) html += `<div style="font-size:10px;color:var(--text-tertiary);padding:4px 8px;">+${results.length-15} más...</div>`;
  ...
  _grpRenderCtx(lineId) {
    ...
    if (matchingRule) {
      ...
      const addInput = container.querySelector('.dw-grp-add-input');
      if (addInput) {
        const previewDiv = document.createElement('div');
        previewDiv.style.cssText = 'margin:4px 0 6px;max-height:160px;overflow-y:auto;';
        addInput.parentNode.insertBefore(previewDiv, addInput.nextSibling);
        this._grpAttachPreview(addInput, previewDiv, bank);
