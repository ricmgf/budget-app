...
const BudgetGrid = {
  ...
  openDrawer(lineId) {
    ...
    ov.innerHTML = `<div class="budget-drawer">
      ...
      <div class="dw-group-section">
        <div class="dw-group-header" onclick="document.getElementById('dw-group-body').classList.toggle('hidden');BudgetGrid._grpRender()">
          <span>🔗 Reglas de agrupación</span><span class="dw-group-chevron">▸</span>
        </div>
        <div id="dw-group-body" class="hidden">
          <div id="dw-grp-list"></div>
          <div class="dw-grp-new-section">
            <div class="dw-grp-new-toggle" onclick="document.getElementById('dw-grp-new-form').classList.toggle('hidden')">+ Crear nuevo grupo</div>
            <div id="dw-grp-new-form" class="hidden">
              <label>Nombre del grupo</label>
              <input id="dw-grp-new-label" placeholder="Ej: Amazon, Peaje...">
              <label>Primer patrón</label>
              <input id="dw-grp-new-pattern" placeholder="Ej: AMAZON, AMZN...">
              <button class="btn-save" style="width:100%;margin-top:8px;" onclick="BudgetGrid._grpCreateNew()">Crear grupo</button>
            </div>
          </div>
        </div>
      </div>
  ...
  _grpRender() {
    const container = document.getElementById('dw-grp-list');
    if (!container) return;
    const bank = this.activeBank;
    const rules = BudgetLogic._groupRules.filter(r => !r.bank || r.bank === bank);
    ...
    if (!groups.size) {
      container.innerHTML = '<div class="dw-grp-empty">No hay reglas de agrupación activas para este banco</div>';
      return;
    }

    let html = '';
    for (const [label, patterns] of groups) {
      html += `<div class="dw-grp-group">
        <div class="dw-grp-group-hdr">
          <strong>${this._e(label)}</strong>
          <span class="dw-grp-count">${patterns.length} patrón${patterns.length > 1 ? 'es' : ''}</span>
        </div>`;
      patterns.forEach(r => {
        html += `<div class="dw-grp-pattern">
          ...
          <button class="dw-grp-del" data-pattern="${this._e(r.pattern)}" data-label="${this._e(r.label)}" onclick="BudgetGrid._grpDeletePattern(this.dataset.pattern)" title="Eliminar patrón">✕</button>
        </div>`;
      });
      html += `<div class="dw-grp-add">
          <input class="dw-grp-add-input" placeholder="Añadir patrón..." data-label="${this._e(label)}">
          <button class="dw-grp-add-btn" onclick="BudgetGrid._grpAddPattern(this)">+</button>
        </div>
      </div>`;
    }
    container.innerHTML = html;

    // Wire up Enter key on add inputs
    container.querySelectorAll('.dw-grp-add-input').forEach(inp => {
      inp.onkeydown = (e) => { if (e.key === 'Enter') this._grpAddPattern(inp.nextElementSibling); };
    });
  },
  ...
  async _grpCreateNew() {
    ...
    if (!pattern) { alert('Introduce al menos un patrón'); return; }

    try {
      await BudgetLogic.createGroupRule(pattern, label, this.activeBank);
      await this._grpApplyToGrid(pattern.toUpperCase(), label);
      this._toast(`Grupo "${label}" creado con patrón "${pattern}"`);
      document.getElementById('dw-grp-new-label').value = '';
      document.getElementById('dw-grp-new-pattern').value = '';
      document.getElementById('dw-grp-new-form')?.classList.add('hidden');
      this._grpRender();
    } catch(e) {
      console.error('Error creating group:', e);
      alert('Error: ' + (e?.result?.error?.message || e.message || 'Error desconocido'));
    }
  },

  async _grpAddPattern(btn) {
  ...
  async _grpDeletePattern(pattern) {
  ...
  async _grpApplyToGrid(pattern, label) {
    ...
    // Find all lines in this bank that match the pattern and are NOT already the group line
    const matching = this.lines.filter(l =>
      l.bank === bank && l.concepto &&
      l.concepto.toUpperCase().includes(pattern) &&
      l.concepto !== label // don't match the group line itself
    );

    if (!matching.length) return;

    // Find or create the group line
    let groupLine = this.lines.find(l => l.bank === bank && l.concepto === label && !l.parentId);

    if (!groupLine) {
      // Create group line in the same section as the first match
      // If the match has a parent (tarjeta child), use the same parentId
      ...
      // Sum all matching lines' real values
      matching.forEach(m => { for (let i = 0; i < 12; i++) amounts[i] += m.real[i]; });
      // Build breakdown from individual lines
      const bd = {};
      matching.forEach(m => {
        for (let i = 0; i < 12; i++) {
          if (m.real[i]) {
      ...
      // Group line exists — add matching amounts to it
      for (let i = 0; i < 12; i++) {
        const addAmt = matching.reduce((s, m) => s + (m.real[i] || 0), 0);
        if (addAmt) {
          groupLine.real[i] = (groupLine.real[i] || 0) + addAmt;
          await BudgetLogic.updateBudgetCell(groupLine.sheetRow, BudgetLogic.getRealCol(i), groupLine.real[i]);
        }
      }
      // Update breakdown
      let bd = {};
      ...
      await BudgetLogic.updateBudgetCell(groupLine.sheetRow, 42, JSON.stringify(bd));
    }

    // Delete the individual matching lines
    for (const m of matching) {
      await BudgetLogic.deleteBudgetLine(m.sheetRow);
    }
