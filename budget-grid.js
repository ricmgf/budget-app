    ...
    ov.innerHTML = `<div class="budget-drawer">
      ...
      <div class="dw-group-section">
        <div class="dw-group-header" onclick="document.getElementById('dw-group-body').classList.toggle('hidden');BudgetGrid._grpRenderCtx('${line.id}')">
          <span>🔗 Crear Agrupación</span><span class="dw-group-chevron">▸</span>
        </div>
    ...
    if (matchingRule) {
      ...
      if (patInput) {
        const updatePreview = () => {
          ...
          const matches = this.lines.filter(l => l.bank === bank && l.concepto && l.concepto.toUpperCase().includes(q) && !l.parentId);
          ...
          for (const [, m] of seen) {
            html += `<label class="dw-grp-match-item"><input type="checkbox" checked disabled><span class="dw-grp-match-name">${this._e(m.concepto)}</span><span class="dw-grp-match-amt">${m.count > 1 ? m.count+'x ' : ''}${this._f(m.total,1)}</span></label>`;
          }
          pv.innerHTML = html;
        };
