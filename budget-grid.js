  ...
  async _impConfirm(type) {
    ...
    try {
      ...
      if (extractoLabel && this._impMovements !== undefined) {
        const dates = (this._impMovements || [])
          .map(m => m.date instanceof Date ? m.date : new Date(m.date))
          .filter(d => d instanceof Date && !isNaN(d))
          .sort((a,b) => a-b);
        const fechaMin = dates.length ? dates[0].toISOString().slice(0,10) : '';
        const fechaMax = dates.length ? dates[dates.length-1].toISOString().slice(0,10) : '';
