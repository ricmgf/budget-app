/**
 * Budget Grid v11.0 — Parser fixes, titular naming, breakdown modal, consolidation groups
 * v10: UTC→local date, Caixa saldo, Intessa V2, bank consolidation, try/catch
 * v11: Iberia CARD-in-Ricardo regex, titulares (X Iberia/Amex/Visa), breakdown modal,
 *      collapse +/−, Visa groups (Super U, Commune, Mercadona), tarjeta child breakdowns
 */
...
const BudgetGrid = {
  ...
  async _impConfirm(type) {
    ...
    const TARJETA_CONSOLIDATION_GROUPS = [
      ...
      { pattern: 'BON. COMI.CAMBIO', label: 'Bon. Comisión Cambio' },
      { pattern: 'BON.COMI.CAMBIO', label: 'Bon. Comisión Cambio' },
      { pattern: 'TSGN', label: 'Tren' },
      { pattern: 'SUPER U', label: 'Super U' },
      { pattern: 'COMMUNE STAT', label: 'Parking Commune' },
      { pattern: 'MERCADONA', label: 'Mercadona' },
      { pattern: 'AMAZON', label: 'Amazon' },
      { pattern: 'AMZN', label: 'Amazon' },
      { pattern: 'IHERB', label: 'iHerb' },
      { pattern: 'EASYJET', label: 'EasyJet' }
    ];
    // Bank import consolidation groups — groups recurring fees/charges under a single label
    const BANK_CONSOLIDATION_GROUPS = [
      { pattern: 'COMMISSIONE', label: 'Commissioni' },
      { pattern: 'COMMISSIONI E SPESE', label: 'Commissioni' },
      { pattern: 'COSTO BONIFICO', label: 'Commissioni' },
      { pattern: 'CANONE MENSILE', label: 'Canone Mensile' }
    ];
