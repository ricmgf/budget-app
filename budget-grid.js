...
const BudgetGrid = {
  ...
  // Preview with checkboxes — auto-fills pattern input from selected results
  _grpAttachPreviewChecked(searchEl, previewEl, patternEl, bank) {
    const MIN  = 2;
    const hint = '<div style="font-size:10px;color:var(--text-tertiary);padding:4px 2px;">Escribe para buscar líneas del extracto...</div>';
    ...
    searchEl.oninput = () => {
      ...
      previewEl.innerHTML = '<div style="font-size:10px;color:var(--text-tertiary);padding:4px 2px;">Buscando...</div>';
      _timer = setTimeout(async () => {
        const lines = await BudgetLogic.searchGastos(q, bank);
        if (!lines.length) {
          previewEl.innerHTML = '<div style="font-size:10px;color:var(--text-tertiary);padding:4px 2px;">Sin coincidencias</div>';
          return;
        }
        let html = `<div style="font-size:10px;color:var(--text-tertiary);margin-bottom:4px;padding:0 2px;">${lines.length} línea${lines.length!==1?'s':''} · selecciona para añadir como patrón</div>`;
        lines.slice(0, 15).forEach(l => {
          ...
          const sub   = l.notas ? String(l.notas).slice(0,50) : '';
          html += `<label style="display:flex;align-items:flex-start;gap:8px;padding:6px 8px;
            background:var(--bg-secondary);border:1px solid var(--border-light);border-radius:5px;
            margin-bottom:3px;cursor:pointer;font-size:11px;line-height:1.3;user-select:none;"
            onclick="event.preventDefault();BudgetGrid._grpPreviewSelect(this,'${this._e(val)}',event)">
            <input type="checkbox" style="margin-top:2px;flex-shrink:0;" value="${this._e(val)}">
            <div style="flex:1;min-width:0;">
              <div style="display:flex;justify-content:space-between;gap:6px;">
                <span style="font-weight:600;color:var(--text-primary);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${this._e(label)}</span>
        ...
        // When checkbox changes, update pattern input with selected concpetos
  ...
  _grpPreviewSelect(labelEl, val, e) {
