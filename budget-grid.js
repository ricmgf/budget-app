...
const BudgetGrid = {
  ...
  openDrawer(lineId) {
    ...
    ov.innerHTML = `<div class="budget-drawer">
      ...
      <div class="dw-group-section">
        <div class="dw-group-header" onclick="document.getElementById('dw-group-body').classList.toggle('hidden')">
          <span>🔗 Agrupar conceptos similares</span><span class="dw-group-chevron">▸</span>
        </div>
        <div id="dw-group-body" class="hidden">
          <div class="dw-group-info">Busca conceptos parecidos y agrúpalos bajo un nombre común. Al re-importar se consolidarán automáticamente.</div>
          <label>Buscar concepto</label>
          <input id="dw-group-search" placeholder="Ej: Amazon, AREA, Super U..." oninput="BudgetGrid._groupSearch(this.value)">
          <div id="dw-group-results"></div>
          <div id="dw-group-create" class="hidden">
            <label>Nombre del grupo</label>
            <input id="dw-group-label" placeholder="Ej: Amazon, Peaje, Supermercado...">
            <label>Banco <span style="font-weight:400;text-transform:none;color:var(--text-tertiary);">(vacío = aplica a todos)</span></label>
            <input id="dw-group-bank" value="${this._e(this.activeBank)}" placeholder="Todos los bancos">
            <button class="btn-save" style="width:100%;margin-top:12px;" onclick="BudgetGrid._groupCreate()">Crear regla de agrupación</button>
          </div>
        </div>
      </div>
  ...
  // ══════════ GROUP CONCEPTS (Drawer) ══════════

  _groupSearch(query) {
    const res = document.getElementById('dw-group-results');
    const create = document.getElementById('dw-group-create');
    if (!res) return;
    if (!query || query.length < 2) { res.innerHTML = ''; create?.classList.add('hidden'); return; }
    const q = query.toUpperCase();
    // Search all lines in current bank for matching conceptos
    const bank = this.activeBank;
    const matches = this.lines
      .filter(l => l.bank === bank && l.concepto && l.concepto.toUpperCase().includes(q))
      .reduce((acc, l) => {
        const key = l.concepto.toUpperCase().trim();
        if (!acc.find(a => a.key === key)) {
          const totalReal = l.real.reduce((s, v) => s + v, 0);
          acc.push({ key, concepto: l.concepto, totalReal, count: 1, section: l.section });
        } else {
    ...
    if (!matches.length) { res.innerHTML = '<div class="dw-group-info">No se encontraron conceptos</div>'; create?.classList.add('hidden'); return; }

    // Check if a group rule already exists for this pattern
    const existingGroup = BudgetLogic._groupRules.find(r => q.includes(r.pattern) || r.pattern.includes(q));

    res.innerHTML = (existingGroup ? `<div class="dw-group-existing">✅ Ya existe regla: "${existingGroup.pattern}" → ${existingGroup.label}</div>` : '') +
      matches.map(m => `<label class="dw-group-item">
        <input type="checkbox" value="${this._e(m.concepto)}" checked>
        <span class="dw-group-concepto">${this._e(m.concepto)}</span>
        <span class="dw-group-amount">${this._f(m.totalReal, 1)}</span>
      </label>`).join('');

    // Show create section
    if (!existingGroup) {
      create?.classList.remove('hidden');
      const labelInput = document.getElementById('dw-group-label');
      if (labelInput && !labelInput.value) labelInput.value = query.trim();
    } else {
  ...
  async _groupCreate() {
    const label = document.getElementById('dw-group-label')?.value?.trim();
    const bank = document.getElementById('dw-group-bank')?.value?.trim() || '';
    const search = document.getElementById('dw-group-search')?.value?.trim();
    if (!label) { alert('Introduce un nombre para el grupo'); return; }
    if (!search) { alert('Introduce un patrón de búsqueda'); return; }

    try {
      await BudgetLogic.createGroupRule(search, label, bank);
      this._toast(`Regla creada: "${search}" → ${label}. Se aplicará en el próximo import.`);
      // Update the search results to show the rule exists now
      this._groupSearch(search);
    } catch(e) {
      console.error('Error creating group rule:', e);
      alert('Error creando regla: ' + (e?.result?.error?.message || e.message || 'Error desconocido'));
    }
  },
  ...
  _parseIntessaV2(rows) {
    ...
    // Don't consolidate fees at parser level — BANK_CONSOLIDATION_GROUPS in _impConfirm
  ...
  async _impConfirm(type) {
    ...
    // Split into tarjeta vs bank-specific groups
    const TARJETA_CONSOLIDATION_GROUPS = allGroupRules.filter(r => !r.bank || r.bank !== this.activeBank || type === 'tarjeta');
    const BANK_CONSOLIDATION_GROUPS = allGroupRules.filter(r => r.bank === this.activeBank);
    ...
    for (const mv of movements) {
      ...
      if (type === 'tarjeta') {
        ...
        // Bank imports: if movement has a titular (e.g. Revolut), use tarjeta-like grouping
        const hasTitular = titular && titular.length > 0;
        const cUp = rawC.toUpperCase();

        if (hasTitular) {
          // Treat like tarjeta: check consolidation groups, then individual rows under titular
          const group = TARJETA_CONSOLIDATION_GROUPS.find(g => cUp.includes(g.pattern));
          if (group) {
            const groupKey = `grp_${titular}_${group.label}`;
            if (consolidated.has(groupKey)) {
          ...
          // Standard bank imports: check bank consolidation groups
          const bankGroup = BANK_CONSOLIDATION_GROUPS.find(g => cUp.includes(g.pattern));

        if (bankGroup) {
          // Consolidate into a single row per group label + section
          const groupKey = `bankgrp_${bankGroup.label}|||${section}`;
          if (consolidated.has(groupKey)) {
            consolidated.get(groupKey).amounts[mi] = (consolidated.get(groupKey).amounts[mi] || 0) + Math.abs(mv.amount);
            if (!breakdowns.get(groupKey)[mi]) breakdowns.get(groupKey)[mi] = [];
            breakdowns.get(groupKey)[mi].push({ d: dateStr, a: Math.abs(mv.amount), c: rawC.substring(0, 50) });
            ...
            const amounts = new Array(12).fill(0);
            amounts[mi] = Math.abs(mv.amount);
            const rule = BudgetLogic.findRuleWithNotes ? BudgetLogic.findRuleWithNotes(bankGroup.label, '', this.activeBank) : BudgetLogic.findRule(bankGroup.label, this.activeBank);
            consolidated.set(groupKey, { concepto: bankGroup.label, section, notes: '', titular: '', amounts, rule });
            const bd = {};
            bd[mi] = [{ d: dateStr, a: Math.abs(mv.amount), c: rawC.substring(0, 50) }];
            breakdowns.set(groupKey, bd);
          }
        } else {
          // Normal bank consolidation: same concepto+notes
    ...
    for (const [key, entry] of consolidated) {
      ...
      // Always include breakdown for group keys (bankgrp_ or grp_) to show child details on hover
      ...
      if (existing) {
        ...
        let parentId = '';
        if (titular) {
          // Check existing parents in the correct section (TARJETAS for tarjeta imports, actual section for bank)
          if (!titularParents.has(titular + '|||' + section)) {
            // Check if parent already exists in budget lines
      ...
      if (count % 3 === 0) showProg();
        ...
        console.error(`[Import] Error importing row ${count}/${total}:`, rowErr);
        count++;
        // Continue with next row — don't let one failure kill the whole import
      }
    }
