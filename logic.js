const BudgetLogic = {
  ...
  async getDashboardData(y, m) {
    const g = await SheetsAPI.readSheet(CONFIG.SHEETS.GASTOS), i = await SheetsAPI.readSheet(CONFIG.SHEETS.INGRESOS);
    const f = (arr, yr, mo) => arr.slice(1).filter(r => r[1] == yr && r[2] == mo && r[0] !== 'DELETED');
    const actG = f(g, y, m), actI = f(i, y, m);
    return { resumen: { totalGastos: actG.reduce((acc, r) => acc + parseFloat(r[5] || 0), 0), totalIngresos: actI.reduce((acc, r) => acc + parseFloat(r[5] || 0), 0) }, pendingCount: actG.filter(r => !r[3] || !r[4]).length };
  }
};
