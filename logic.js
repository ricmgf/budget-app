...
const BudgetLogic = {
  ...
  async _retroactiveCategorizeLine(rule) {
    ...
    BudgetGrid.lines.forEach(line => {
      ...
      // Match pattern against concepto + notas (same logic as findRuleWithNotes)
      const haystack = this._normalize(line.concepto + ' ' + (line.notas || ''));
      const parts = rule.pattern.split('|||');
      const matched = parts.every(p => { const n = this._normalize(p); return n && haystack.includes(n); });
      if (!matched) return;
