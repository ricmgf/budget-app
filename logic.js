...
const BudgetLogic = {
  ...
  findRuleWithNotes(concepto, notes, bankName) {
    ...
    for (const rule of this._rules) {
      ...
      if (rule.bank && rule.bank !== bankName) continue;
      if (ruleNotes) {
        // For rules with |||: concepto must match as EXACT substring only — no wordMatch.
        // This prevents PAGAMENTO BOLLETTA CBILL from matching PAGAMENTO ADUE via shared word.
        const cMatch = normC.includes(ruleConcepto) || ruleConcepto.includes(normC);
        if (!cMatch) continue;
        // Notes: exact substring first, then wordMatch only if keyword is long enough (>=8 chars)
        // to avoid single short words like "DISP" matching unrelated notes.
        const nMatch = normN.includes(ruleNotes) ||
                       (ruleNotes.length >= 8 && this._wordMatch(ruleNotes, normN));
        if (nMatch) return rule;
      } else {
        // No notes pattern: exact substring on concepto only, no notes involved
        if (normC.includes(ruleConcepto) || ruleConcepto.includes(normC)) return rule;
        // wordMatch on concepto only (not combined with notes) — prevents RICARDO in notes
        // from matching a concepto rule like 'Visa Débito Ricardo'
        if (this._wordMatch(ruleConcepto, normC)) return rule;
      }
    }
