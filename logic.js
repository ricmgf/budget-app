...
const BudgetLogic = {
  ...
  // ─── Gastos cache — shared across Budget drawer and Rules page ───────────────
  _gastosCache: {},   // { [bank]: rows[] }

  async loadGastosForBank(bank) {
    if (this._gastosCache[bank]) return this._gastosCache[bank];
    try {
      const rows = await SheetsAPI.readSheet(CONFIG.SHEETS.GASTOS);
      const G = GASTOS_COLS;
      // All rows for this bank (skip header)
      const bankRows = (rows || []).slice(1).filter(r =>
        r && r[G.ID] !== undefined && r[G.ID] !== '' && r[G.CUENTA] === bank
      );
      this._gastosCache[bank] = bankRows;
      return bankRows;
    } catch(e) {
      console.warn('[BudgetLogic] loadGastosForBank error:', e);
      return [];
    }
  },

  invalidateGastosCache(bank) {
    if (bank) delete this._gastosCache[bank];
    else this._gastosCache = {};
  },

  // Normalise: lowercase, remove accents, collapse spaces
  _norm(s) {
  ...
  // Simple fuzzy: every word in query must appear as substring OR
  // match with at most 1 char difference (for words 5+ chars)
  _fuzzyMatch(haystack, query) {
    ...
    return words.every(w => {
      ...
      // Sliding window edit-distance-1 check
  ...
  // Search GASTOS_TOTAL for bank, returns deduplicated results grouped by concepto
  // Each result: { concepto, count, total, rows }
  async searchGastos(query, bank) {
    if (!query || query.trim().length < 2) return [];
    const rows = await this.loadGastosForBank(bank);
    const G = GASTOS_COLS;
    const q = query.trim();

    const matched = rows.filter(r => {
      return this._fuzzyMatch(r[G.CONCEPTO], q) ||
             this._fuzzyMatch(r[G.NOTAS],    q) ||
             this._fuzzyMatch(r[G.ORIGEN],   q);
    });

    // Deduplicate by normalised concepto
    const map = new Map();
    matched.forEach(r => {
      const key = this._norm(r[G.CONCEPTO]);
      const imp = parseFloat(r[G.IMPORTE]) || 0;
      if (map.has(key)) {
        map.get(key).count++;
        map.get(key).total += imp;
        map.get(key).rows.push(r);
      } else {
        map.set(key, { concepto: r[G.CONCEPTO] || key, count: 1, total: imp, rows: [r] });
      }
    });
