...
const BudgetLogic = {
  ...
  async loadAccounts() {
    ...
    const accounts = (rows || []).slice(1).filter(r => r[0] && r[0] !== 'DELETED').map((r, i) => ({
      name: r[0], iban: r[1] || '', casa: r[2] || '', tarjetas: r[3] || '',
      order: parseInt(r[4]) || (i + 1), row: i + 2
    }));
  ...
  async createRule(concepto, bankName, casa, categoria, subcategoria) {
    ...
    // Create new (or replace one with unknown sheetRow)
    if (existing) {
      const idx = this._rules.indexOf(existing);
