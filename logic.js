const BudgetLogic = {
  async loadConfig() {
    // Lectura fresca para sincronizar cambios realizados desde la Web
    const rows = await SheetsAPI.readSheet(CONFIG.SHEETS.CONFIG);
    const cfg = { categorias: {}, cuentas: [], casas: [] };
    
    rows.slice(1).forEach(row => {
      // Filtrado Enterprise: Ignoramos filas marcadas como DELETED en la columna 5
      if (row[0] && row[4] !== 'DELETED') { 
        if (!cfg.categorias[row[0]]) cfg.categorias[row[0]] = []; 
        if (row[1] && row[1] !== '' && row[1] !== 'General') cfg.categorias[row[0]].push(row[1]); 
      }
  ...
  // --- LEGACY INTACTO: Sniffer, Hash, Fechas y Dashboard ---
  ...
  async getDashboardData(y, m) {
    ...
    return { totalGastos: actG.reduce((a,b) => a + (parseFloat(b[5])||0), 0), totalIngresos: actI.reduce((a,b) => a + (parseFloat(b[5])||0), 0), plannedGastos: planG.reduce((a,b) => a + (parseFloat(b[3])||0), 0), fundingPlan: funding, pendingCount: g.filter(r => r[12] === 'Pendiente').length };
  }
};
