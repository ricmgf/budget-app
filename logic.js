...
const BudgetLogic = {
  ...
  async loadBudgetLines(year) {
    ...
    // Start at i=0: BUDGET_LINES has no header row — data starts at Sheet row 1
    // (rows[0]). The old i=1 start silently skipped Sheet row 1 forever.
    const idCount = {};
  ...
  // dedupSkipped: cuántas filas se omitieron por deduplicación (diagnóstico).
  async registerImport(bank, statementLabel, year, month, filename, rowsImported, fechaMin, fechaMax, dedupSkipped = 0) {
    ...
    await SheetsAPI.appendRow(CONFIG.SHEETS.IMPORTED_STATEMENTS, [
      ...
      now, filename, rowsImported, fechaMin, fechaMax, dedupSkipped
    ]);
  },
