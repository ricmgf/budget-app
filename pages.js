    ...
    c.innerHTML = `
      <div class="metric-grid">
        <div class="card" onclick="navigateTo('review')" style="cursor:pointer">
          <h3>Inbox</h3><h2 style="color:var(--accent)">${d.pendingCount} <span style="font-size:16px; font-weight:400;">items</span></h2>
        </div>
        <div class="card"><h3>Neto Mes</h3><h2 class="${d.cashFlow >= 0 ? 'positive' : 'negative'}">${Utils.formatCurrency(d.cashFlow)}</h2></div>
        <div class="card"><h3>Diferencia Plan</h3><h2 class="${(d.plannedGastos - d.totalGastos) >= 0 ? 'positive' : 'negative'}">${Utils.formatCurrency(d.plannedGastos - d.totalGastos)}</h2></div>
      </div>
      <div class="two-col-equal">
          ...
          <div style="margin-top:8px;">
            ${Object.entries(d.fundingPlan).map(([acc, amt]) => `
              <div style="display:flex; justify-content:space-between; padding:16px 0; border-bottom:1px solid var(--border-light);">
                <span style="font-weight:500;">Mover a <strong>${acc}</strong></span>
        ...
        <div class="card">
          ...
          <div style="margin-top:16px;">
            <p style="display:flex; justify-content:space-between; margin-bottom:12px;"><span>Previsto In:</span> <strong>${Utils.formatCurrency(d.plannedIngresos)}</strong></p>
  ...
  c.innerHTML = `
    <div class="decision-queue" style="max-width:550px; margin:auto;">
      <div class="card" style="border: 2px solid var(--accent);">
        <h2 style="margin:0 0 12px 0;">${item[GASTOS_COLS.CONCEPTO]}</h2>
        <h1 style="margin:0 0 32px 0;">${Utils.formatCurrency(item[GASTOS_COLS.IMPORTE])}</h1>
        ...
        <div class="form-group" style="margin-top:28px;"><label>Categoría</label><div class="chip-group" id="cat-chips">${Object.keys(AppState.config.categorias).map(cat => `<button class="chip" onclick="selectChip(this, 'cat')">${cat}</button>`).join('')}</div></div>
        <button style="width:100%; margin-top:40px; padding:22px; background:var(--accent); color:#fff; border:none; border-radius:16px; font-weight:700; cursor:pointer;" onclick="resolveItem('${item[GASTOS_COLS.CONCEPTO]}')">ENTRENAR REGLA</button>
      </div>
    </div>`;
}
...
function loadStubPage(p) { document.getElementById(`${p}-content`).innerHTML = `<div class="card"><h3>${p}</h3><p>En desarrollo.</p></div>`; }
