 ...
 * FIX: Exposición global para evitar "ReferenceError"
 */
const AppState = {
...
function renderCategoriasTab(container, header) {
  ...
  Object.keys(cats).forEach(cat => {
    html += `<div class="settings-row" style="flex-direction:column; align-items:flex-start; gap:12px; background:white; padding:16px; border-radius:12px; border:1px solid var(--border-light); margin-bottom:12px;">
      ...
      <div style="display:flex; flex-wrap:wrap; gap:8px;">
        ${cats[cat].map(s => `<span class="tag-card" style="background:#f1f5f9; padding:4px 12px; border-radius:20px; font-size:12px;">${s} <button onclick="deleteSubcategory('${cat}','${s}')" style="background:none; border:none; color:var(--negative); cursor:pointer;">×</button></span>`).join('')}
...
function renderBancosTab(container, header) {
  SheetsAPI.readSheet(CONFIG.SHEETS.ACCOUNTS).then(accs => {
    ...
    if (AppState.isAddingBank) {
      ...
      html += `<div class="settings-row" style="background:#f8fafc; padding:20px; border-radius:12px; margin-bottom:20px; display:grid; grid-template-columns: repeat(4, 1fr) auto; gap:12px; align-items:end;">
    ...
    html += `<table style="width:100%; text-align:left; border-collapse:collapse;">
      <thead><tr style="font-size:12px; color:var(--text-secondary); border-bottom:1px solid var(--border-light);"><th>NOMBRE</th><th>IBAN</th><th style="text-align:right;">ACCIONES</th></tr></thead>
...
function renderCasasTab(container, header) {
  container.innerHTML = `${header}<div style="display:flex; justify-content:space-between; margin-bottom:24px;"><h3>Casas</h3><button onclick="addCasaMaster()" style="background:var(--accent); color:white; border:none; padding:8px 16px; border-radius:8px; cursor:pointer;">+ Nueva Casa</button></div>
    <div style="display:grid; gap:12px;">
      ${AppState.config.casas.map(c => `<div class="settings-row" style="background:white; padding:16px; border-radius:12px; border:1px solid var(--border-light); display:flex; justify-content:space-between;"><span>${c.name}</span><button onclick="deleteCasaMaster(${c.row})" style="background:none; border:none; color:var(--negative); font-weight:700; cursor:pointer;">Eliminar</button></div>`).join('')}
    </div>`;
}
...
window.renameCategoryMaster = async function(old) { 
  const n = prompt("Nuevo nombre para la categoría:", old); 
  if (n && n !== old) { alert("Esta función requiere mapeo de filas Col A. Se recomienda borrar y crear."); } 
};
window.deleteCategoryMaster = async function(cat) { 
  if (confirm(`¿Eliminar la categoría "${cat}"?`)) { alert("Requiere borrado físico en Sheet. Use el Excel para esta versión."); } 
};

window.addSubcategory = async function(cat) { 
  const n = prompt(`Nueva subcategoría para ${cat}:`); 
  if (n) { await SheetsAPI.appendRow(CONFIG.SHEETS.CONFIG, [cat, n]); await BudgetLogic.loadConfig(); loadSettingsPage(); } 
};
window.deleteSubcategory = async function(cat, sub) { 
  if (confirm(`¿Eliminar la subcategoría "${sub}"?`)) { alert("Requiere localización de fila. Use el Excel."); } 
};
