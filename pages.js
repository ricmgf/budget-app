// ============================================================
// Budget App — Master UI Controller (v1.11 - LEGACY COMPLETO)
// ============================================================
...
// --- NAVEGACIÓN (7 MENÚS FUNCIONALES) ---
function navigateTo(p) {
  ...
  else loadStubPage(p); // Para Rules, Reporting, Balances, etc.
}

// --- DASHBOARD (Sincronización Inmediata) ---
      ...
      <div class="two-col-equal">
        ...
        <div class="card">
          ...
          <div style="margin-top:20px;">
            <p>Real: ${Utils.formatCurrency(d.totalGastos)}</p>
            <p>Presupuestado: ${Utils.formatCurrency(d.plannedGastos)}</p>
          </div>
        </div>
      </div>`;
  } catch(e) { 
    console.error(e);
    c.innerHTML = `<div class="card" style="color:red">Error al sincronizar datos del Dashboard.</div>`;
  }
}

// --- SETTINGS (Gestor de Bancos CRUD) ---
        ...
        <table style="width:100%; text-align:left; border-collapse:collapse; margin-top:20px;">
          <thead>
            <tr style="color:var(--text-secondary); font-size:12px; border-bottom:1px solid var(--border-light);">
              ...
              <th style="padding:10px;">ID (IBAN/CARD)</th>
          ...
          <tbody>
            ${accs.slice(1).map(a => `
              <tr style="border-bottom:1px solid #f1f5f9;">
                <td style="padding:10px;"><strong>${a[0]}</strong></td>
                <td style="padding:10px; font-family:monospace;">${a[1]}</td>
                <td style="padding:10px;">${a[2]}</td>
                <td style="padding:10px;"><span class="badge">${a[3]}</span></td>
              </tr>`).join('')}
          </tbody>
        </table>
        <div style="margin-top:30px; padding-top:20px; border-top:1px solid var(--border-light);">
          <h4>Añadir cuenta nueva</h4>
          <div style="display:grid; grid-template-columns: 1fr 1fr 1fr 1fr; gap:10px; margin-top:10px;">
            <input type="text" id="new-acc-alias" placeholder="Alias (ej: Caixa)" style="padding:8px; border:1px solid #ddd; border-radius:8px;">
            <input type="text" id="new-acc-id" placeholder="IBAN o ID" style="padding:8px; border:1px solid #ddd; border-radius:8px;">
            <select id="new-acc-casa" style="padding:8px; border:1px solid #ddd; border-radius:8px;">
              ${casas.map(cas => `<option value="${cas}">${cas}</option>`).join('')}
            </select>
            <select id="new-acc-type" style="padding:8px; border:1px solid #ddd; border-radius:8px;">
              <option value="Current">Corriente</option>
              <option value="Credit">Crédito</option>
            </select>
          </div>
          <button onclick="saveNewAccount()" style="margin-top:15px; padding:10px 20px; background:var(--accent); color:white; border:none; border-radius:8px; cursor:pointer; font-weight:700;">Guardar Banco</button>
        </div>
      </div>`;
  } catch (e) { 
    c.innerHTML = '<div class="card" style="color:red">Error al cargar la hoja ACCOUNTS. Asegúrate de que existe.</div>'; 
  }
}
...
async function saveNewAccount() {
  ...
  if(!row[0] || !row[1]) return alert("Alias e Identificador son obligatorios");
  await SheetsAPI.appendRow(CONFIG.SHEETS.ACCOUNTS, row);
  loadSettingsPage();
}

// --- REVIEW (Legacy Decision Queue con Chips) ---
async function loadReviewPage() {
  ...
  if (pending.length === 0) {
    c.innerHTML = '<div class="card" style="text-align:center; padding:80px;"><h3>Inbox Zero 🎉</h3></div>';
    return;
  }
  ...
  c.innerHTML = `
    <div class="decision-queue" style="max-width:550px; margin:auto;">
      <div class="card" style="border: 2px solid var(--accent);">
        <h3 style="margin-bottom:5px;">${item[4]}</h3>
        <h2 style="font-size:32px; margin-bottom:20px;">${Utils.formatCurrency(item[5])}</h2>
        
        <div class="form-group">
          <label>Propiedad / Casa</label>
        ...
        <button style="width:100%; margin-top:30px; padding:20px; background:var(--accent); color:#fff; border:none; border-radius:12px; font-weight:700; cursor:pointer;" onclick="resolveItem('${item[4]}')">ENTRENAR REGLA Y GUARDAR</button>
      </div>
    </div>`;
}
...
async function resolveItem(p) {
  ...
  await BudgetLogic.saveRuleAndApply(p, cat, "", casa);
  loadReviewPage();
}
...
function loadImportPage() {
  document.getElementById('import-content').innerHTML = `
    <div class="card">
      <div class="upload-zone" onclick="document.getElementById('f').click()" style="padding:80px; text-align:center; cursor:pointer; background:var(--bg-canvas); border-radius:24px; border:2px dashed var(--border-light);">
        <h3>📥 Importar Movimientos</h3>
        <p>Sube tu XLSX o CSV. El sistema detectará el banco automáticamente.</p>
      </div>
...
async function handleFileImport(e) {
  ...
  reader.onload = async (evt) => {
    ...
    alert(`Banco Detectado: ${res.account}. Importados: ${res.imported}.`);
    navigateTo('review');
  };
...
function loadStubPage(p) {
  ...
  if (cont) cont.innerHTML = `<div class="card"><h3>${p}</h3><p style="color:var(--text-secondary);">Módulo ${p} en desarrollo para el próximo sprint.</p></div>`;
}
...
// --- INICIALIZACIÓN (Solución al Dashboard Blanco) ---
async function initApp() {
  AppState.init(); 
  try {
    // Forzamos la carga de Config antes de navegar
    AppState.config = await BudgetLogic.loadConfig();
    navigateTo('dashboard');
  } catch(e) {
    console.error("Fallo al inicializar app:", e);
  }
}

// Llamada inmediata para pintar el mes antes del login
updateMonthSelector();
