  ...
  Object.entries(byBank).sort((a,b) => a[0].localeCompare(b[0])).forEach(([bank, lines]) => {
    ...
    Object.entries(byConcepto).sort((a,b) => b[1].total - a[1].total).forEach(([concepto, grp]) => {
      ...
      rows += `<tr data-sincat-row="${safeId}" style="border-bottom:1px solid var(--border-light);">
        ...
        <td style="padding:8px;min-width:150px;"><select id="sc-cat-${safeId}" onchange="scCatChange(this,'${safeId}')" style="${selStyle}">${catOpts('')}</select></td>
        <td style="padding:8px;min-width:150px;"><select id="sc-sub-${safeId}" style="${selStyle}">${subOpts('','')}</select></td>
