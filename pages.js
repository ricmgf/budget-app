/**
 * [MASTER_ARCH_V2.0.1_PROTEGIDO]
 * REGLA DE ORO: NO MUTILAR. ARRANQUE PRESERVADO.
 * MODIFICACIONES: Mapeo explícito de Columna E (Tarjetas) y selector dinámico.
 */
  ...
  SheetsAPI.readSheet(CONFIG.SHEETS.ACCOUNTS).then(accs => {
    ...
    if (AppState.isAddingBank) {
      ...
      html += `<div style="background:var(--bg-canvas); padding:24px; border-radius:12px; margin-bottom:24px; display:grid; grid-template-columns: repeat(2, 1fr); gap:20px;">
          ...
          <button onclick="saveBank()" style="background:var(--positive); color:white; border:none; padding:12px 24px; border-radius:8px; cursor:pointer; font-weight:700; grid-column: span 2;">GUARDAR DATOS</button>
        </div>`;
    }
...
window.deleteBankMaster = async function(row) {
  if (confirm("¿Eliminar banco?")) { await SheetsAPI.updateCell(CONFIG.SHEETS.ACCOUNTS, row, 1, 'DELETED'); loadSettingsPage(); }
};

window.addCardMaster = async function() {
  const n = prompt("Nombre tarjeta (Columna E):");
  if (n) { alert("Agrega '" + n + "' en la Columna E del Excel y refresca."); }
};

window.deleteCardMaster = async function(row) {
  if (confirm("¿Eliminar?")) { await SheetsAPI.updateCell(CONFIG.SHEETS.CONFIG, row, 5, 'DELETED'); await BudgetLogic.loadConfig(); loadSettingsPage(); }
};
...
window.deleteCasaMaster = async function(row) {
  if (confirm("¿Eliminar?")) { await SheetsAPI.updateCell(CONFIG.SHEETS.CONFIG, row, 4, 'DELETED'); await BudgetLogic.loadConfig(); loadSettingsPage(); }
};
