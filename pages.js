...
function _rulesAttachPreview(inputEl, previewEl) {
  const MIN  = 2;
  const hint = '<span style="font-size:10px;color:var(--text-tertiary);">Escribe 2+ letras para buscar en transacciones</span>';
  ...
  inputEl.oninput = () => {
    ...
    _timer = setTimeout(async () => {
      ...
      let results = [];
      if (bank) {
        results = await BudgetLogic.searchGastos(q, bank);
      } else {
        const banks = (AppState.config?.accounts || []).map(a => a.name);
        const all = await Promise.all(banks.map(b => BudgetLogic.searchGastos(q, b)));
        const map = new Map();
        all.flat().forEach(r => {
          const k = BudgetLogic._norm(r.concepto);
          if (map.has(k)) { map.get(k).count += r.count; map.get(k).total += r.total; }
          else map.set(k, { ...r });
        });
        results = [...map.values()].sort((a, b) => b.total - a.total);
      }
      if (!results.length) {
      ...
      let html = '<div style="font-size:10px;color:var(--text-tertiary);margin-bottom:4px;">'
        + total + ' transacción' + (total !== 1 ? 'es' : '') + ' encontradas'
        + (bank ? ' en ' + bank : '') + '</div>';
      results.slice(0, 15).forEach(r => {
        const amt = r.total.toLocaleString('es-ES', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
        html += '<div style="display:flex;justify-content:space-between;align-items:center;'
          + 'padding:4px 8px;background:white;border:1px solid var(--border-light);'
          ...
          + _esc(r.concepto) + '</span>'
          + '<span style="color:var(--text-tertiary);flex-shrink:0;">'
          + (r.count > 1 ? r.count + '× ' : '')
          + '<strong style="color:var(--text-secondary);">' + amt + '</strong></span></div>';
      });
      if (results.length > 15) {
        html += '<div style="font-size:10px;color:var(--text-tertiary);padding:4px 8px;">+'
          + (results.length - 15) + ' más...</div>';
      }
