// ============================================================
// Budget App — Master UI Controller (v1.5)
// ============================================================
...
async function loadDashboard() {
  ...
  // Cambio solicitado: Sincronizando datos
  ...
  try {
    ...
    c.innerHTML = `<div class="card" style="color:var(--negative)">Error al cargar datos. Verifica la conexión.</div>`; 
  }
}
...
async function loadSettingsPage() {
  ...
  try {
      ...
      <div class="card">
        ...
        <div style="margin-top:30px; padding-top:20px; border-top:1px solid var(--border-light);">
          ...
          <div style="display:grid; grid-template-columns: 1fr 1fr 1fr 1fr; gap:10px; margin-top:10px;">
            <input type="text" id="new-acc-alias" placeholder="Ej: Caixa" style="padding:8px; border-radius:6px; border:1px solid var(--border-light);">
    ...
    c.innerHTML = `<div class="card">Error al cargar bancos. Asegúrate de que la hoja ACCOUNTS existe.</div>`;
  }
}
...
function loadStubPage(p) { 
  ...
  if (cont) cont.innerHTML = `<div class="card"><h3>${p}</h3><p style="color:var(--text-secondary);">Módulo en desarrollo.</p></div>`; 
}
...
// INICIALIZACIÓN CRÍTICA: Primero carga config, luego Dashboard
async function initApp() {
  AppState.init(); 
  try { 
    // Aseguramos que loadConfig se llame correctamente
    AppState.config = await BudgetLogic.loadConfig(); 
    console.log("Configurada AppState:", AppState.config);
