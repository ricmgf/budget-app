...
async function loadSettingsPage() {
  ...
  // Rules: must be GASTOS section, no categoria, not a child line, not deleted
  // TARJETAS lines (headers and children) are managed from the grid drawer, not here
  let uncatLines = [];
  try {
    const allLines = await BudgetLogic.getAllLines();
    uncatLines = allLines.filter(l =>
      !l.categoria &&
      l.section === 'GASTOS' &&   // only GASTOS — excludes all TARJETAS and INGRESOS
      !l.parentId                  // not a child/grouped line
    );
  } catch(e) {}
...
function renderSinCategorizarTab(container, header, uncatLines, cats, casas) {
  ...
  Object.entries(byBank).sort((a,b) => a[0].localeCompare(b[0])).forEach(([bank, lines]) => {
    ...
    rows += `<tr><td colspan="6" style="background:var(--bg-secondary,#f8fafc);padding:8px 12px;font-size:11px;font-weight:700;color:var(--text-secondary);letter-spacing:.06em;text-transform:uppercase;border-top:2px solid var(--border-light);border-bottom:1px solid var(--border-light);">${bank}</td></tr>`;
      ...
      ${uncatLines.length === 0
        ...
        <table style="width:100%;border-collapse:collapse;font-size:13px;">
          <thead><tr style="border-bottom:2px solid var(--border-light);background:var(--bg-secondary,#f8fafc);">
            ...
            <th style="padding:10px 8px;text-align:right;font-weight:700;font-size:11px;color:var(--text-secondary);text-transform:uppercase;letter-spacing:.04em;">Total</th>
