  ...
  Object.entries(byBank).sort((a,b) => a[0].localeCompare(b[0])).forEach(([bank, lines]) => {
    ...
    Object.entries(byConcepto).sort((a,b) => b[1].total - a[1].total).forEach(([concepto, grp]) => {
      ...
      rows += `<tr data-sincat-row="${safeId}">
        <td style="padding:8px 12px;font-size:13px;max-width:200px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" title="${concepto}">${concepto}</td>
        <td style="padding:8px;font-size:12px;color:var(--text-secondary);">${grp.lines.length} línea${grp.lines.length>1?'s':''}</td>
        <td style="padding:8px;font-size:13px;font-weight:600;text-align:right;">${grp.total.toLocaleString('es-ES',{minimumFractionDigits:2})} €</td>
