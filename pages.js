...
function renderBancosTab(container, header, casas, tarjetas) {
  SheetsAPI.readSheet(CONFIG.SHEETS.ACCOUNTS).then(accs => {
    ...
    for (let i = 1; i < accs.length; i++) {
      const a = accs[i];
      if (a[0] && a[0] !== 'DELETED') {
        banks.push({ name: a[0]||'', iban: a[1]||'', casa: a[2]||'', tarjeta: a[3]||'', sheetRow: i + 1, order: parseInt(a[4]) || i });
      }
    }
    ...
    html += `<table style="width:100%; border-collapse:collapse; text-align:left;">
          <thead><tr style="color:var(--text-secondary); font-size:11px; text-transform:uppercase; letter-spacing:0.05em; border-bottom: 1px solid var(--border-light);">
            <th style="padding:12px 8px; font-weight:700; width:36px;">Orden</th><th style="font-weight:700;">Banco</th><th style="font-weight:700;">IBAN</th><th style="font-weight:700;">Tarjetas</th><th style="font-weight:700;">Casa</th><th style="text-align:right; font-weight:700;">Acciones</th></tr>
          </thead>
          <tbody>`;
    ...
    banks.forEach((bk, idx) => {
      ...
      html += `<tr style="border-bottom:1px solid var(--border-light);" data-row="${bk.sheetRow}">
        ...
        <td style="text-align:right;">
          <button onclick="initEditBank(${bk.sheetRow}, '${safeName}', '${safeIban}', '${safeCasa}', '${safeCards}')" style="background:none; border:none; color:var(--accent); cursor:pointer; font-weight:600; margin-right:12px;">Editar</button>
...
window.saveBank = async function() {
  ...
  try {
    if (AppState.editingBankData && AppState.editingBankData.row) {
      ...
      await SheetsAPI.updateRow(CONFIG.SHEETS.ACCOUNTS, AppState.editingBankData.row, [n, i, c, t]);
    } else {
      // New bank — add with order = 999 (goes to end)
      await SheetsAPI.appendRow(CONFIG.SHEETS.ACCOUNTS, [n, i, c, t, 999]);
    }
...
window.renameGroup = async function(oldLabel) {
  ...
  try {
    // Find all rules with this label and update them
    const rows = await SheetsAPI.readSheet(CONFIG.SHEETS.RULES);
    if (!rows) return;
    for (let i = 1; i < rows.length; i++) {
      if (String(rows[i][3]||'').trim() === 'group' && String(rows[i][4]||'').trim() === oldLabel) {
        await SheetsAPI.updateCell(CONFIG.SHEETS.RULES, i + 1, 5, newLabel);
      }
    }
    // Update in-memory
