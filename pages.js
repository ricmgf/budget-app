// ============================================================
// Budget App — Master UI Controller (Complete State)
// ============================================================
  ...
  try {
    ...
    c.innerHTML = `
      <div class="metric-grid">
        <div class="card" onclick="navigateTo('review')" style="cursor:pointer"><h3>Inbox</h3><h2 style="color:var(--accent)">${d.pendingCount} items</h2></div>
        <div class="card"><h3>Neto Mes</h3><h2 class="${d.cashFlow >= 0 ? 'positive' : 'negative'}">${Utils.formatCurrency(d.cashFlow)}</h2></div>
        <div class="card"><h3>Variance</h3><h2 class="${(d.plannedGastos - d.totalGastos) >= 0 ? 'positive' : 'negative'}">${Utils.formatCurrency(d.plannedGastos - d.totalGastos)}</h2></div>
      </div>
      <div class="two-col-equal">
        ...
        <div class="card"><h3>📈 Forecast</h3><p>Previsto In: ${Utils.formatCurrency(d.plannedIngresos)}</p><p>Previsto Out: ${Utils.formatCurrency(d.plannedGastos)}</p></div>
      </div>`;
...
async function loadReviewPage() {
  ...
  c.innerHTML = '<div style="padding:40px; text-align:center;">Cargando Queue...</div>';
  ...
  c.innerHTML = `
    <div class="decision-queue" style="max-width:500px; margin:auto;">
      <div class="card" style="border: 2px solid var(--accent);">
        <h2>${item[GASTOS_COLS.CONCEPTO]}</h2>
...
function loadStubPage(p) { document.getElementById(`${p}-content`).innerHTML = `<div class="card"><h3>${p}</h3><p>Módulo en desarrollo.</p></div>`; }
