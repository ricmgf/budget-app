...
async function loadDashboard() {
  ...
  try {
    const d = await API.getDashboard({ año: AppState.currentYear, mes: AppState.currentMonth });
...
async function processFile(file) {
  ...
  try {
    ...
    const r = await API.importTransactions({ fileContent: b64, fileName: file.name, cuenta: cuenta || undefined });
    if (r.needsManualSelection) {
      result.innerHTML = `<div class="alert alert-warning">Could not auto-detect format. Select account:</div>
        <select class="form-select mb-4" id="manual-imp-cuenta">${Utils.buildCuentaSelect(AppState.config,'')}</select>
        <button class="btn btn-primary" id="retry-btn">Retry</button> <button class="btn btn-secondary" onclick="loadImportPage()">Cancel</button>`;
      window._pendingB64 = b64; window._pendingFN = file.name;
      document.getElementById('retry-btn').onclick = async()=>{
        ...
        try{const r2=await API.importTransactions({fileContent:window._pendingB64,fileName:window._pendingFN,cuenta:ct});showImportResult(r2,result);}
...
async function saveRev(id) {
  ...
  try {
    await API.updateGasto({id, categoria:cat, subcategoria:sub, casa});
...
function showBulkCat() {
  ...
  `, async()=>{
    ...
    try{await API.bulkUpdateGastos({ids,categoria:cat,subcategoria:sub,casa});closeModal();Utils.showAlert(`${ids.length} updated`,'success');loadReviewPage();}
    catch(err){Utils.showAlert(err.message,'danger');}
  });
}
function onBCatChg() {
...
function showRuleFromTxn(id, concepto) {
  ...
  `, async()=>{
    ...
    try{const r=await API.addRule(data);if(r.id!==undefined)await API.applyRuleRetroactive(r.id);closeModal();Utils.showAlert(r.message,'success');
...
async function prevRule(){
  ...
  try{const r=await API.testRule({patron:document.getElementById('rp').value,tipoMatch:document.getElementById('rt').value,cuenta:document.getElementById('rcu').value});
...
async function toggleRule(id,en){try{await API.updateRule({id,habilitado:en});loadRulesPage();}catch(e){Utils.showAlert(e.message,'danger');}}
async function delRule(id){if(!confirm('Delete rule?'))return;try{await API.deleteRule({id});loadRulesPage();}catch(e){Utils.showAlert(e.message,'danger');}}
...
async function runTest(){
  ...
  try{const r=await API.testRule({patron:document.getElementById('tp').value,tipoMatch:document.getElementById('tt').value});
...
async function loadReportingPage() {
  ...
  try {
    const [mo, ca] = await Promise.all([
      API.getMonthlySummary(AppState.currentYear, AppState.currentMonth),
      API.getCasaSummary(AppState.currentYear, AppState.currentMonth)
    ]);
    c.innerHTML = `
...
async function loadAnnual() {
  ...
  try {
    const d = await API.getAnnualSummary(AppState.currentYear);
...
async function exportCSV() {
  try { const r = await API.exportGastos({año:AppState.currentYear}); Utils.downloadCSV(r.csv,r.filename); Utils.showAlert('Downloaded','success'); }
  catch(err) { Utils.showAlert(err.message,'danger'); }
}
...
async function loadBalancesPage() {
  ...
  c.innerHTML = '<div class="loading-overlay"><div class="spinner"></div></div>';
  try {
    const d = await API.calculateCashFlow({ año:AppState.currentYear, mes:AppState.currentMonth });
...
async function editBalances() {
  const {balances} = await API.getBalances({año:AppState.currentYear,mes:AppState.currentMonth});
  ...
  showModal('Opening Balances — '+AppState.getMonthName(AppState.currentMonth)+' '+AppState.currentYear,
    ...
    async()=>{
      try{for(const c of AppState.config.cuentas){
        await API.updateBalance({año:AppState.currentYear,mes:AppState.currentMonth,cuenta:c,saldoInicial:parseFloat(document.getElementById(`bl-${c.replace(/\s/g,'_')}`).value)||0});}
...
async function editIngresos() {
  const {ingresos} = await API.getIngresos({año:AppState.currentYear,mes:AppState.currentMonth});
...
async function saveIng(btn) {
  ...
  try{await API.addIngreso({año:AppState.currentYear,mes:AppState.currentMonth,concepto:row.querySelector('[data-f="concepto"]').value,
...
async function delIng(id) {
  ...
  try{await API.deleteIngreso({id});document.querySelector(`[data-iid="${id}"]`).remove();Utils.showAlert('Deleted','success');}catch(e){Utils.showAlert(e.message,'danger');}
}

// --- SETTINGS ---
async function loadSettingsPage() {
  if(!AppState.config)await AppState.loadConfig();
...
function addCatDlg(){
  ...
  showModal('Add Category',`<div class="form-group"><label class="form-label">Categoría</label><input class="form-input" id="nc" list="ncl" placeholder="Existing or new"><datalist id="ncl">${cats.map(c=>`<option value="${c}">`).join('')}</datalist></div>
    ...
    async()=>{try{await API.addCategory(document.getElementById('nc').value,document.getElementById('ns').value);closeModal();await AppState.loadConfig();loadSettingsPage();Utils.showAlert('Added','success');}catch(e){Utils.showAlert(e.message,'danger');}});
}
async function delCat(cat,sub){if(!confirm(`Delete "${cat}/${sub}"?`))return;try{await API.deleteCategory(cat,sub);await AppState.loadConfig();loadSettingsPage();Utils.showAlert('Deleted','success');}catch(e){Utils.showAlert(e.message,'danger');}}
function addCuentaDlg(){showModal('Add Account',`<div class="form-group"><label class="form-label">Name</label><input class="form-input" id="ncu"></div>`,
  async()=>{try{await API.addCuenta(document.getElementById('ncu').value);closeModal();await AppState.loadConfig();loadSettingsPage();Utils.showAlert('Added','success');}catch(e){Utils.showAlert(e.message,'danger');}});}
function addCasaDlg(){showModal('Add Property',`<div class="form-group"><label class="form-label">Name</label><input class="form-input" id="nca"></div>`,
  async()=>{try{await API.addCasa(document.getElementById('nca').value);closeModal();await AppState.loadConfig();loadSettingsPage();Utils.showAlert('Added','success');}catch(e){Utils.showAlert(e.message,'danger');}});}
...
function showManualEntry(){
  ...
  showModal('Add Manual Expense',`
    ...
    async()=>{try{await API.addManualGasto({fecha:document.getElementById('me-f').value,concepto:document.getElementById('me-c').value,importe:document.getElementById('me-i').value,
...
async function initApp() {
  ...
  try { await AppState.loadConfig(); navigateTo('dashboard'); }
  catch(err) {
    document.getElementById('page-dashboard').classList.add('active');
    document.getElementById('dashboard-content').innerHTML=`<div class="alert alert-danger">
      <strong>Connection Error</strong><br>Could not connect to backend. Check:<br>
      1. Apps Script Web App is deployed<br>2. URL in api.js is correct<br>3. Logged in with authorized account<br><br>
