...
const Reporting = (() => {
  ...
  function renderTarjetas() {
    ...
    // All TARJETAS lines in range
    const allTarjetaLines = _lines.filter(l => l.section === 'TARJETAS');

    // Parent lines = tarjeta header rows (parentId empty), children = individual charges
    const parentLines   = allTarjetaLines.filter(l => !l.parentId);
    const childrenLines = allTarjetaLines.filter(l => !!l.parentId);

    if (!configTarjetas.length && !parentLines.length) {
      return '<p class="rpt-empty">Sin tarjetas configuradas. Ve a Ajustes > Tarjetas para añadirlas.</p>';
    }

    // Build display name list from config (sorted alpha), add any parents not in config
    const parentNames   = [...new Set(parentLines.map(l => l.alias || l.concepto))];
    const allNames      = [...new Set([
      ...configTarjetas,
      ...parentNames.filter(n => !configTarjetas.includes(n))
    ])].sort((a,b) => a.localeCompare(b,'es'));
    ...
    selNames.forEach(name => {
      // Find the parent line(s) matching this card name
      const cardParents = parentLines.filter(l => (l.alias || l.concepto) === name);

      // Sum across all active months (parent real[] + children of these parents)
      const allCardLines = [
        ...cardParents,
        ...childrenLines.filter(c => cardParents.some(p => p.id === c.parentId))
      ];

      // For display: sum only parent real[] values (parents already aggregate children in sheet)
      const vals = ms.map(m => cardParents.reduce((s, l) => s + (l.real[m.month - 1] || 0), 0));

      if (vals.every(v => v === 0)) {
        // Card in config but no data this period
      ...
      // Detail: show children (individual charges) when available, else parent
      const detailLines = childrenLines.filter(c => cardParents.some(p => p.id === c.parentId));
      const txns = linesToTxns(detailLines.length ? detailLines : cardParents);
