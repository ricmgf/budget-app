...
const Reporting = (() => {
  ...
  function renderTarjetas() {
    ...
    // Reset if stored selection has stale values
    _selTarjetas = _selTarjetas.filter(n => allNames.includes(n));
    if (!_selTarjetas.length) _selTarjetas = [...allNames];
    ...
    selNames.forEach(name => {
      ...
      // Detail: show children charges when available; fallback to parent if no children
      const detailLines = cardChildren.length ? cardChildren : cardParents;
      const txns = linesToTxns(detailLines);
