...
const Reporting = (() => {
      ...
      if (parsed) {
        ...
        Object.entries(parsed).forEach(([mIdx, arr]) => {
          ...
          (arr || []).forEach(t => {
            txns.push({
              mes: t.d || label,
              concepto: t.c || l.alias || l.concepto,
              cuenta: l.bank,
  ...
  function renderCasas() {
    ...
    // Casas from config or derived from data
    const configCasas = (AppState.config?.casas || []).map(c => c.name);
    const dataCasas   = [...new Set(gastos.map(l => l.casa).filter(Boolean))];
    const casas = configCasas.length ? configCasas.filter(c => dataCasas.includes(c)) : dataCasas.sort();
    if (!casas.length) return '<p class="rpt-empty">No hay gastos con Casa asignada.</p>';
    ...
    const cats = [...configCats.filter(c => dataCats.includes(c)), ...dataCats.filter(c => !configCats.includes(c))];
    ...
    return `<div class="rpt-table-wrap"><table class="rpt-table">${buildThead('Concepto', colDefs)}<tbody>${tbody}</tbody></table></div>`;
  }
  ...
  function renderCategorias() {
    ...
    if (!_catSel) return selHtml + '<p class="rpt-empty">Sin datos.</p>';
    const catLines = gastos.filter(l => l.categoria === _catSel);
    if (!catLines.length) return selHtml + '<p class="rpt-empty">Sin transacciones para esta categoría.</p>';
    ...
    return selHtml + `<div class="rpt-table-wrap"><table class="rpt-table">${buildThead('Subcategoría', colDefs)}<tbody>${tbody}</tbody></table></div>`;
  }
  ...
  function renderHijos() {
    ...
    if (!hijoCats.length) return '<p class="rpt-empty">No se encontraron categorías de hijos.</p>';
    ...
    if (_hijosView === 'anual') {
      ...
      return `<div class="rpt-table-wrap"><table class="rpt-table">${buildThead('Hijo', colDefs)}<tbody>${tbody}</tbody></table></div>`;
    }
  }
  ...
  function renderTarjetas() {
    const ms      = activeMonths();
    const tarjetas = _lines.filter(l => l.section === 'TARJETAS' &&
      ms.some(m => (l.real[m.month - 1] || 0) !== 0));
    const COLSPAN  = ms.length + 2;
    const colDefs  = ms.map(m => ({ label: monthLabel(m) }));

    // Group by bank (each bank is a "tarjeta" section)
    const banks = [...new Set(tarjetas.map(l => l.bank))].sort();
    if (!banks.length) return '<p class="rpt-empty">Sin datos de tarjetas en el período.</p>';

    const totals = new Array(ms.length).fill(0);
    let tbody = '';

    banks.forEach(bank => {
      const bLines = tarjetas.filter(l => l.bank === bank);
      const vals   = ms.map(m => sumReal(bLines, m.month));
      if (vals.every(v => v === 0)) return;
      vals.forEach((v, i) => totals[i] += v);
      const bankId = uid();
      let tds = vals.map(v => `<td class="rpt-td rpt-hi">${fmt(v)}</td>`).join('');
      tds += `<td class="rpt-td rpt-hi">${fmt(vals.reduce((a,b)=>a+b,0))}</td>`;
      tbody += `<tr class="rpt-cat-row" onclick="Reporting.toggleSec('${bankId}')">
        <td class="rpt-td rpt-td-first">${xbtn(bankId)}${bank}</td>${tds}
      </tr>`;
      // Individual lines within this bank's tarjetas
      bLines.forEach(l => {
        const lVals = ms.map(m => l.real[m.month - 1] || 0);
        if (lVals.every(v => v === 0)) return;
        const lId  = uid();
        const txns = linesToTxns([l]);
        let lTds = lVals.map(v => `<td class="rpt-td">${fmt(v)}</td>`).join('');
        lTds += `<td class="rpt-td rpt-hi">${fmt(lVals.reduce((a,b)=>a+b,0))}</td>`;
        tbody += `<tr class="rpt-sub-row" data-g="${bankId}" style="display:none"
          onclick="event.stopPropagation();Reporting.toggle('${lId}')">
          <td class="rpt-td rpt-td-sub">${xbtn(lId)}${l.alias||l.concepto}</td>${lTds}
        </tr>`;
    ...
    return `<div class="rpt-table-wrap"><table class="rpt-table">${buildThead('Tarjeta / Línea', colDefs)}<tbody>${tbody}</tbody></table></div>`;
  }
  ...
  function render() {
    ...
    // ── Tab bar (same style as budget nav-tabs) ──
    const tabsHtml = `<div class="budget-bank-tabs" style="padding:0 24px;border-bottom:1px solid var(--border-light);background:white;">
      ${tabs.map(t => `<button class="budget-bank-tab${_tab===t?' active':''}" onclick="Reporting.switchTab('${t}')">${tabLabel(t)}</button>`).join('')}
    </div>`;
    ...
    ctrlRight += `<button class="ctl${_view===2?' on':''}" onclick="Reporting.setView(2)">2m</button>`;
    ctrlRight += `<button class="ctl${_view===12?' on':''}" onclick="Reporting.setView(12)">12m</button>`;
    ctrlRight += `<button class="date-range-btn" onclick="Reporting.toggleDatePicker()">📅 <span>${fmtDateRange()}</span> ▾</button>`;
    ctrlRight += `<button class="btn btn-primary" onclick="Reporting.reload()" style="padding:5px 14px;font-size:12px;">↺ Recargar</button>`;
    ...
    const stickyHtml = `
      <div class="rpt-sticky">
        ...
        <div class="rpt-datepicker" id="rpt-dp" style="display:none;align-items:center;gap:12px;padding:10px 24px;background:#f8faff;border-bottom:1px solid var(--border-light);">
          <label style="font-size:12px;font-weight:600;">Desde</label>
          <input type="date" id="rpt-from" class="ctl" value="${_dateFrom.toISOString().slice(0,10)}" style="padding:5px 10px;">
          <label style="font-size:12px;font-weight:600;">Hasta</label>
          <input type="date" id="rpt-to"   class="ctl" value="${_dateTo.toISOString().slice(0,10)}"   style="padding:5px 10px;">
          <button class="btn btn-primary" onclick="Reporting.applyDates()" style="padding:5px 14px;font-size:12px;">Aplicar</button>
    ...
    document.getElementById('rpt-wrap').innerHTML = stickyHtml
      + `<div style="padding:20px 24px 60px;">${tableHtml}</div>`;
  }
