...
const Reporting = (() => {
  ...
  function renderTarjetas() {
    ...
    // Source of truth for card names: config.tarjetas (Ajustes > Tarjetas)
    const configTarjetas = (AppState.config?.tarjetas || []).map(t => t.name);

    // All TARJETAS lines with real data in range
    const allTarjetaLines = _lines.filter(l => l.section === 'TARJETAS' &&
      ms.some(m => (l.real[m.month - 1] || 0) !== 0));

    if (!configTarjetas.length && !allTarjetaLines.length) {
      return '<p class="rpt-empty">Sin tarjetas configuradas. Ve a Ajustes > Tarjetas para añadirlas.</p>';
    }

    // Merge: config names (sorted alpha) + any extra from data not in config
    const dataNames = [...new Set(allTarjetaLines.map(l => l.alias || l.concepto))];
    const allNames  = [...new Set([...configTarjetas, ...dataNames.filter(n => !configTarjetas.includes(n))])]
      .sort((a,b) => a.localeCompare(b,'es'));
    ...
    selNames.forEach(name => {
      const cardLines = allTarjetaLines.filter(l => (l.alias || l.concepto) === name);
      const vals      = ms.map(m => cardLines.reduce((s, l) => s + (l.real[m.month - 1] || 0), 0));
      if (vals.every(v => v === 0)) {
        // Configured tarjeta but no data this period — show greyed row
