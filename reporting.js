...
const Reporting = (() => {
  ...
  ───────────────────────────────────────────── */
  function buildMultiSelect(msId, label, allItems, selected, onChange) {
    ...
    const opts = allItems.map(item => {
      ...
      return `<label class="rpt-ms-opt">
        <input type="checkbox" ${chk} onchange="Reporting.msToggle('${msId}','${safe}',${JSON.stringify(allItems)},${onChange})">
        <span>${item}</span>
      </label>`;
    }).join('');

    return `<div class="rpt-ms" id="ms-${msId}">
      ...
      <div class="rpt-ms-wrap">
        ...
        <div class="rpt-ms-panel" id="ms-panel-${msId}">
          <label class="rpt-ms-opt rpt-ms-all">
            <input type="checkbox" id="ms-all-${msId}" ${allSel?'checked':''} onchange="Reporting.msToggleAll('${msId}',${JSON.stringify(allItems)},${onChange},this.checked)">
  ...
  function renderCasas() {
    ...
    const allCasas    = configCasas.length ? configCasas.filter(c => dataCasas.includes(c)) : dataCasas.sort();
    ...
    if (!casas.length) return buildMultiSelect('casas', 'Casas', allCasas, _selCasas, '"casas"') + '<p class="rpt-empty">Selecciona al menos una casa.</p>';
    ...
    return buildMultiSelect('casas', 'Casas', allCasas, _selCasas, '"casas"') +
      `<div class="rpt-table-wrap"><table class="rpt-table">${buildThead('Concepto', colDefs)}<tbody>${tbody}</tbody></table></div>`;
  }
  ...
  function renderCategorias() {
    ...
    const configCats = Object.keys(AppState.config?.categorias || {});
    const dataCats   = [...new Set(gastos.map(l => l.categoria).filter(Boolean))];
    const allCats    = [...configCats, ...dataCats.filter(c => !configCats.includes(c))];

    if (!_selCategorias) _selCategorias = [...allCats];
    const selCats = allCats.filter(c => _selCategorias.includes(c));

    const msHtml = buildMultiSelect('categorias', 'Categoría', allCats, _selCategorias, '"categorias"');
    ...
    return msHtml + `<div class="rpt-table-wrap"><table class="rpt-table">${buildThead('Categoría / Subcategoría', colDefs)}<tbody>${tbody}</tbody></table></div>`;
  }
  ...
  function renderHijos() {
    ...
    const msHtml   = buildMultiSelect('hijos', 'Hijos', allHijos, _selHijos, '"hijos"');
    ...
    if (_hijosView === 'anual') {
      ...
      return msHtml + `<div class="rpt-table-wrap"><table class="rpt-table">${buildThead('Concepto', colDefs)}<tbody>${tbody}</tbody></table></div>`;
      ...
      return msHtml + `<div class="rpt-table-wrap"><table class="rpt-table">${buildThead('Hijo', colDefs)}<tbody>${tbody}</tbody></table></div>`;
    }
  }
  ...
  function renderTarjetas() {
    ...
    if (!tarjetas.length) return '<p class="rpt-empty">Sin datos de tarjetas en el período.</p>';

    // Each line IS a tarjeta — group by alias||concepto
    const allNames = [...new Set(tarjetas.map(l => l.alias || l.concepto))].sort();
    if (!_selTarjetas) _selTarjetas = [...allNames];
    const selNames = allNames.filter(n => _selTarjetas.includes(n));
    const msHtml   = buildMultiSelect('tarjetas', 'Tarjeta', allNames, _selTarjetas, '"tarjetas"');
    ...
    selNames.forEach(name => {
      // Aggregate all lines that share this name (can be multiple banks/rows)
      const cardLines = tarjetas.filter(l => (l.alias || l.concepto) === name);
      const vals      = ms.map(m => cardLines.reduce((s, l) => s + (l.real[m.month - 1] || 0), 0));
      if (vals.every(v => v === 0)) return;
      vals.forEach((v, i) => totals[i] += v);
      const cardId = uid();
      const txns   = linesToTxns(cardLines);
      let tds = vals.map(v => `<td class="rpt-td rpt-hi">${fmt(v)}</td>`).join('');
      tds += `<td class="rpt-td rpt-hi">${fmt(vals.reduce((a,b)=>a+b,0))}</td>`;
    ...
    return msHtml + `<div class="rpt-table-wrap"><table class="rpt-table">${buildThead('Tarjeta', colDefs)}<tbody>${tbody}</tbody></table></div>`;
  }
  ...
  function renderIngresos() {
    ...
    const allConceptos = [...new Set(ing.map(l => l.alias || l.concepto))];
    if (!_selIngresos) _selIngresos = [...allConceptos];
    const selConceptos = allConceptos.filter(c => _selIngresos.includes(c));
    const msHtml = buildMultiSelect('ingresos', 'Concepto', allConceptos, _selIngresos, '"ingresos"');
  ...
  return {
    ...
    msTogglePanel(msId) {
      ...
      const close = (e) => {
        if (!document.getElementById(`ms-${msId}`)?.contains(e.target)) {
    ...
    msToggle(msId, item, allItems, tabKey) {
      let sel = _getTabSel(tabKey) || [...allItems];
      const idx = sel.indexOf(item);
      if (idx >= 0) sel.splice(idx, 1); else sel.push(item);
      _setTabSel(tabKey, sel.length === allItems.length ? null : sel);
      // Update "select all" checkbox
      const allChk = document.getElementById(`ms-all-${msId}`);
      if (allChk) allChk.checked = sel.length === allItems.length;
      render();
    ...
    msToggleAll(msId, allItems, tabKey, checked) {
      _setTabSel(tabKey, checked ? null : []);
      render();
      const panel = document.getElementById(`ms-panel-${msId}`);
      if (panel) panel.classList.add('open');
    },
